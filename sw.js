...
async function verifyCore(){const cache=await caches.open(CACHE),missing=[];for(const path of CORE){if(!await cache.match(path))missing.push(path);}const keys=await caches.keys(),stale=keys.filter(k=>k.startsWith('atelier-space-studio-')&&k!==CACHE);return {type:'ATELIER_SW_STATUS',version:RELEASE,cache:CACHE,core:CORE,ready:missing.length===0,missing,stale};}
async function clearStale(){const keys=await caches.keys(),stale=keys.filter(k=>k.startsWith('atelier-space-studio-')&&k!==CACHE);await Promise.all(stale.map(k=>caches.delete(k)));return stale;}
